...
function createAdder(howMuchToAdd){
    function addThem(whatToAdd) {
        return whatToAdd + howMuchToAdd;
    }
    return addThem
};
...
// Exercise 2 - Promises 
// Using the below getMovieInfo function, which is a Promise-based function, write an asynchronous function (.then().catch() or async/await)
...
function printMovieInfo(movieName){
    getMovieInfo(movieName)
        .then(movie => console.log(`${movieName} directed by ${movie.director}. A story of ${movie.description} that runs for ${movie.runtime} minutes.`))
